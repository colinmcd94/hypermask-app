...
export const getWallet = async () => {
  ...
  if (localStorage.encryptedHypermaskVault) {
    state = await passworder.decrypt(GLOBALS.WALLET_PASSWORD, localStorage.encryptedHypermaskVault);
  } else {
    state = {
...
export const setWallet = async state => {
  localStorage.encryptedHypermaskVault = await passworder.encrypt(GLOBALS.WALLET_PASSWORD, state);
};
