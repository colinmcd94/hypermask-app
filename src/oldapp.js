    ...
    await parallel(
        ...
        async () => {
            if (txParams.chainId === undefined)
                txParams.chainId = web3.utils.numberToHex(
...
function roundUSD(usdAmount) {
    return usdAmount > 100 ? Math.ceil(usdAmount) : usdAmount.toFixed(2);
}
    ...
    return makeFlow(
        async function(...args) {
            ...
            if (app.state.fail) {
                ...
                app.state.fail(
                    "Transaction cancelled by newer transaction request"
                );
            }
            let ctx = this;
            Object.assign(this, {
                setState(obj) {
                    ...
                    if (app.state.fail !== ctx.fail)
                        ctx.fail("Flow has already failed");
                    setState(obj);
                },
            ...
            try {
                ...
                if (eventName)
                    Event(eventName + " aborted (" + err.message + ")");
                hasError = err;
            } finally {
                if (!hasError)
                    (async () => {
...
const rpcMethods = {
    ...
    personal_sign: interactive(async function(message, from) {
        await interactiveSignatureRequest.call(
            this,
    ...
    eth_signTypedData: interactive(async function(message, from, extraParams) {
        await interactiveSignatureRequest.call(
            this,
            message
                .map(k => k.name + ": " + JSON.stringify(k.value, null, "  "))
                .join("\n")
        );
    ...
    eth_sendTransaction: interactive(async function(txParams) {
        ...
        console.assert(
            (await getWallet()).getAddressString() ===
        ...
        let priceEstimate = BN(txParams.value).add(
            BN(txParams.gasPrice).mul(BN(txParams.gas))
        );
        ...
        let isERC20 =
            /70a08231/.test(contractCode) && /a9059cbb/.test(contractCode); // balanceOf(address) // transfer(address,uint256)
        ...
        if (isERC20) {
            try {
                ...
                var tokenName = await ERC20.methods.name().call(),
                    ...
                    tokenBalance = await ERC20.methods
                        .balanceOf(txParams.from)
                        .call(),
            ...
            if (tokenName) {
                ...
                let tokenMethodParams = _.fromPairs(
                    txData.params.map(k => [k.name, k.value])
                );
                ...
                this.setState({
                    ...
                    insufficientTokens: BN(tokenMethodParams._value).gt(
                        BN(tokenBalance)
                    ),
                    tokenName: tokenName,
        ...
        if (sufficientLeftovers) {
            ...
            let usdAmount = Math.max(
                1,
                roundUSD(ether * ethUSDPrice * PRICE_VOLATILITY_BUFFER)
            );
...
window.addEventListener(
    ...
    function(event) {
        if (
            ...
            event.data &&
            event.data.app === "hypermask-reply" &&
            origin === event.origin
        ) {
            let data = event.data;
            ...
            {state.screen === "finish" ? null : (
                <div className="footer">
                    <div
                        ...
                        style={{ flex: 1 }}
                        onClick={e =>
                            state.fail("Transaction cancelled by user")
                        }
                    >
                        {abort}
                    </div>
...
function IdentityBody() {
    return (
        <div className="body" key="identify">
            ...
            <p className="caveat">
                If you allow this request, this app will be able to see your
                current account balance and transaction history.
            </p>
            <p className="caveat">
                Rejecting this request may limit your ability to use this
                application. It will not be able to request funds or signatures.
            </p>
        </div>
    );
}
function StallBody() {
    return (
        <div key="stall" className="body">
            ...
            <p className="caveat">
                We've opened a new page where you can purchase ETH via{" "}
                <b>Coinbase</b>. Buy the requested amount of ETH with your{" "}
                <b>Debit Card</b> or <b>Bank Account</b>.
            </p>
            <p className="caveat">
                Once you've finished purchasing the ETH, <b>close that tab</b>{" "}
                and the transaction will continue from here.
            </p>
        </div>
    );
}
    ...
    return (
        <div className={"body spinner wait-" + state.phase}>
            ...
            <p className="caveat" style={{ marginTop: -20 }}>
                ...
                <a target="_blank" href={explore(state.myAddress)}>
            ...
            <p className="caveat">
                ...
                your purchased ETH will stay in HyperMask for use in future
                transactions.
            </p>
        </div>
    );
...
function WidgetHeader() {
    return (
        <div
            className={
                "header" + (chain.slug === "mainnet" ? " mainnet" : " testnet")
            }
        >
            <div className="name">
                <a
...
function SignBody() {
    return (
        <div className="body">
            <h2>Sign Message</h2>
            <p>
                This app has requested <b>your signature</b> on the following
                message:
            </p>
...
function TokenBody() {
    ...
    return (
        <div className="body">
            <h2>
                Transfer{" "}
                <a target="_blank" href={explore(state.to)}>
                    <u>{state.tokenName}</u>
                </a>
            </h2>
            <p>
            ...
            {state.insufficientTokens
                ? [
                      <p style={{ color: "red" }}>
                          <b>
                              You do not have enough {state.tokenName} to
                      ...
                      <p>
                          ...
                          <b>
                              {" "}
                              {d(
                                  BN(state.tokenMethodParams._value).sub(
                                      BN(state.tokenBalance)
                                  )
                              )}{" "}
                              {state.tokenSymbol}{" "}
                          </b>
                          ...
                          and try again. You may be able to purchase through a
                          token exchange such as{" "}
                          <a
                              href="https://forkdelta.github.io/"
                              target="_blank"
                          >
                              <b>ForkDelta</b>
                          </a>.
                ...
                : [
                      <p>
                          You have enough <b>{state.tokenName}</b> to complete
                          this transaction with
                          <b>
                              {" "}
                              {d(
                                  BN(state.tokenBalance).sub(
                                      BN(state.tokenMethodParams._value)
                                  )
                              )}{" "}
                              {state.tokenSymbol}{" "}
                          </b>
                      ...
                      <p>
                          You will also need <Price wei={state.priceEstimate} />{" "}
...
function Price(props) {
    ...
    if (app.state.ethUSDPrice) {
        ...
        if (BN(state.priceEstimate).gt(BN(state.currentBalance))) {
            value = Math.max(
                1,
                roundUSD(ether * state.ethUSDPrice * PRICE_VOLATILITY_BUFFER)
            );
        } else {
...
function LeftoverBody() {
    ...
    return (
        <div className="body">
            ...
            <p>
                <Recipient /> has requested <Price wei={state.priceEstimate} />{" "}
                to continue.
            </p>
            <p>
                You have <Price wei={state.currentBalance} /> available in
                leftover funds— enough to complete this transaction with{" "}
                <Price
                    wei={BN(state.currentBalance).sub(state.priceEstimate)}
                />{" "}
                to spare.
            </p>
        </div>
    ...
    return (
        <div className="body">
            ...
            {BN(state.currentBalance).isZero() ? (
                <p>
                    <Recipient /> has requested{" "}
                    <Price wei={state.priceEstimate} buffer /> to continue.
                </p>
            ) : (
                <p>
                    <Recipient /> has requested{" "}
                    <Price wei={state.priceEstimate} />, but due to leftover
                    funds, only{" "}
                    <Price
                        buffer
                        wei={state.priceEstimate.sub(BN(state.currentBalance))}
                    />{" "}
...
function RightArrow() {
    return (
        <svg width="23" height="20" viewBox="0 0 28 24" version="1.1">
            <g id="Canvas" transform="translate(-1447 -11449)">
                <g id="Vector 2">
                    <use
                        href="#path0_stroke"
                        transform="translate(1447.24 11449.8)"
                        fill="#2F80ED"
                    />
                </g>
            </g>
            <defs>
                <path
                    id="path0_stroke"
                    d="M 25.5956 10.3415L 26.3955 10.9417L 26.8726 10.3059L 26.3608 9.69767L 25.5956 10.3415ZM 0 11.3415L 25.5956 11.3415L 25.5956 9.34154L 0 9.34154L 0 11.3415ZM 26.3608 9.69767L 17.6583 -0.643865L 16.1281 0.643865L 24.8305 10.9854L 26.3608 9.69767ZM 24.7958 9.74134L 16.0934 21.3387L 17.6931 22.5391L 26.3955 10.9417L 24.7958 9.74134Z"
                />
            </defs>
        </svg>
    );
...
function CheckMark() {
    return (
        <svg width="18" height="18" viewBox="0 0 27 27" version="1.1">
            <g id="Canvas" transform="translate(7015 -14011)">
                <g id="Vector 2">
                    <use
                        href="#path0_stroke"
                        transform="translate(-7013.44 14012.5)"
                        fill="#27AE60"
                    />
                </g>
            </g>
            <defs>
                <path
                    id="path0_stroke"
                    d="M 7.85463 22.7141L 6.7948 23.7756L 8.06397 25.0428L 9.08679 23.5696L 7.85463 22.7141ZM 22.3922 -0.855448L 6.62247 21.8587L 9.08679 23.5696L 24.8565 0.855448L 22.3922 -0.855448ZM 8.91446 21.6527L 1.05983 13.8104L -1.05983 15.9333L 6.7948 23.7756L 8.91446 21.6527Z"
                />
            </defs>
        </svg>
    );
        ...
        return (
            ...
            >
                ...
                {this.props.clock ? (
                    <text x="125" y="100">
                        {formatTime(
                            Math.round((Date.now() - start) / 1000) + 1
                        )}
                    </text>
                ) : null}
            </svg>
...
function Dashboard() {
    ...
    document.body.className =
        "dashboard" + (chain.slug === "mainnet" ? " mainnet" : " testnet");
        ...
        <div className={"main"}>
            ...
            <div className="block">
                ...
                <div className="caveat">
                    HyperMask maintains a local Ethereum wallet in your browser—
                    its private keys never leave your computer. Funds are
                    briefly sent from Coinbase to your HyperMask wallet, after
                    which the funds are immediately spent on the requested
                    transaction. Value may be left over in your HyperMask wallet
                    due to changes in Ethereum's price during the purchase
                    process, if a transaction fails and gets refunded by the
                    network, or if ETH is sent to your wallet address (for
                    instance from contract earnings).
                </div>
                <div className="caveat">
                    As your HyperMask wallet is accessible to anyone with
                    physical access to your computer, you should not use
                    Hypermask to hold substantial amounts of ether. Think of
                    HyperMask's wallet as a jar for storing loose change, rather
                    than a bank vault for storing your life savings.
                </div>
            </div>
            <div className="block">
                <div>
                        ...
                        onClick={() => {
                            ...
                            input.onchange = function() {
                                ...
                                fr.onload = async function() {
                                    ...
                                    let restoredWallet = Wallet.fromPrivateKey(
                                        restoredPK
                                    );
                                    let restoredBalance = BN(
                                    ...
                                    if (
                                        ...
                                        alert(
                                            "No need to import your current wallet."
                                        );
                                        ...
                                        let gasValue = BN(txObj.gasPrice).mul(
                                            BN(txObj.gas)
                                        );
                                        txObj.value = restoredBalance.sub(
                                            gasValue
                                        );

                                        let newBalance = txObj.value.add(
                                            currentBalance
                                        );
                                        ...
                                        if (
                                            ...
                                            tx.sign(
                                                restoredWallet.getPrivateKey()
                                            );
                                            const serializedTx = tx.serialize();
                                            const signedTx = ethUtil.bufferToHex(
                                                serializedTx
                                            );
                ...
                <div className="caveat">
                    All wallet funds associated with this browser will be
                    irrevocably lost if you clear website data associated with
                    hypermask.io without first saving a backup of your private
                    key.
                </div>

                {BN(state.currentBalance || "0").isZero() ? null : (
                    <div>
                        <button
                        ...
                        {!web3.givenProvider ? null : (
                            ...
                            >
                                <b>Transfer</b> to{" "}
                                {/Metamask/.test(
                                    web3.givenProvider.constructor.name
                                )
                                    ? "MetaMask"
                                    : "Native"}{" "}
                                Wallet
                ...
                <div className="caveat">
                    Transferring your account balance does not automatically
                    transfer ERC20 tokens and other account-linked crypto
                    assets, if you have any of them you must manually export &
                    import the HyperMask private key.
                </div>
            </div>
            ...
            <div className="block">
                <div>
                    <label>
                        <input
                            ...
                            onChange={e => {
                                localStorage.requireIdentityApproval = e.target
                                    .checked
                                    ? "true"
                                    : "";
                                setState({});
                        ...
                        />
                        Require approval before sharing identity with
                        decentralized apps
                    </label>
                </div>
                <div className="caveat">
                    By default, the public account address for your HyperMask
                    Ethereum wallet is automatically to any decentralized app.
                    This is the same behavior of the MetaMask browser extension,
                    but this means that any website can read your pseudonymous
                    transaction history and account balance. If this box is
                    checked, HyperMask shares no information with apps until you
                    explicitly agree to.
                </div>
                ...
                <div>
                    <label>
                        <input
                            ...
                            onChange={e => {
                                localStorage.disableAnalytics = e.target.checked
                                    ? "true"
                                    : "";
                                setState({});
                            }}
                ...
                <div className="caveat">
                    By default, Google Analytics is used to collect anonymous
                    usage information about HyperMask. If this box is checked,
                    HyperMask will not report any analytics information.
                </div>
            </div>
        </div>
    );
