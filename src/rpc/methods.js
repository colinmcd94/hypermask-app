...
export async function eth_accounts() {
  if (!sessionDeanonymized) {
    try {
      let flow = new Flow("eth_accounts", async function() {
...
export const personal_sign = (message, from) => {
  let flow = new Flow("personal_sign", async function() {
...
export const eth_sign = (from, message) => {
  let flow = new Flow("eth_sign", async function() {
    await flowutil.requestSignature.call(this, web3.utils.hexToAscii(message));
    const serialized = sigUtil.personalSign(await walletutil.getPrivateKey(from), {
      from: from,
      data: message
...
export const eth_signTypedData = (message, from, extraParams) => {
  let flow = new Flow("signTypedData", async function() {
...
export const eth_sendTransaction = txParams => {
  let flow = new Flow("eth_sendTransaction", async function() {
