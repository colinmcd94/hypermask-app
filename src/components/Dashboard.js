    ...
    <div className={"main"}>
      ...
      <div className="block">
        ...
        <div className="caveat">
          ...
          transaction fails and gets refunded by the network, or if ETH is sent to your wallet
          address (for instance from contract earnings).
        </div>
        <div className="caveat">
          ...
          wallet as a jar for storing loose change, rather than a bank vault for storing your life
          savings.
        </div>
      </div>
      <div className="block">
        <div>
          <button
            ...
            onClick={async () => {
              gautil.send("Download Private Key");
              ...
              input.onchange = function() {
                ...
                fr.onload = async function() {
                  ...
                  let restoredBalance = util.BN(
                    await web3.eth.getBalance(restoredWallet.getAddressString(), "pending")
                  );

                  let wallet = await walletutil.getWallet();
                  let currentBalance = util.BN(
                    await web3.eth.getBalance(wallet.getAddressString(), "pending")
                  );

                  if (wallet.getAddressString() === restoredWallet.getAddressString()) {
                    gautil.send("Restore Current Wallet");
                    ...
                    if (
                      ...
                      gautil.send("Replace Empty Wallet", restoredBalance);
                      walletutil.setWallet(restoredState);
                      // alert('Current (empty) wallet replaced with imported wallet.')
                      updateDashboard();
                    } else {
                      gautil.send("Replace Empty Wallet (Cancel)", restoredBalance);
                    }
                  } else {
                    // we transfer the balance of the old key into the new key
                    if (restoredBalance.isZero()) {
                      gautil.send("Import Empty Wallet", restoredBalance);
                    ...
                    if (
                      confirm(
                        `Do you want to transfer ${web3.utils.fromWei(
                          ...
                          gasValue,
                          "ether"
                        )} ETH (${(100 * gasValue.div(restoredBalance).toNumber()).toFixed(
                          4
                      ...
                      gautil.send("Import Wallet", restoredBalance);
                      ...
                      gautil.send("Import Wallet (Cancel)", restoredBalance);
                    }
                  }
        ...
        <div className="caveat">
          All wallet funds associated with this browser will be irrevocably lost if you clear
          website data associated with hypermask.io without first saving a backup of your private
          key.
        </div>
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (
                  ...
                  gautil.send("Donate (Sent)", currentBalance);
                  ...
                  gautil.send("Donate (Cancel)", currentBalance);
                }
              }}
            >
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (
                    ...
                    gautil.send("Transfer MetaMask (Sent)", currentBalance);
                    ...
                    gautil.send("Transfer MetaMask (Cancel)", currentBalance);
                  }
                }}
              >
                <b>Transfer</b> to{" "}
                {/Metamask/.test(web3.givenProvider.constructor.name) ? "MetaMask" : "Native"}{" "}
                Wallet
              </button>
            )}
          </div>
        ...
        <div className="caveat">
          Transferring your account balance does not automatically transfer ERC20 tokens and other
          account-linked crypto assets, if you have any of them you must manually export & import
          the HyperMask private key.
        </div>
      </div>
      ...
      <div className="block">
        ...
        <div className="caveat">
          By default, the public account address for your HyperMask Ethereum wallet is automatically
          to any decentralized app. This is the same behavior of the MetaMask browser extension, but
          this means that any website can read your pseudonymous transaction history and account
          balance. If this box is checked, HyperMask shares no information with apps until you
          explicitly agree to.
        </div>
        ...
        <div className="caveat">
          By default, Google Analytics is used to collect anonymous usage information about
          HyperMask. If this box is checked, HyperMask will not report any analytics information.
        </div>
      </div>
    </div>
  );
