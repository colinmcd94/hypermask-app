...
export async function eth_accounts() {
  if (!sessionDeanonymized) {
    try {
      await modalutil.interactive(async function() {
      ...
      event.send("Request Account ID (Denied)");
      return [];
    }
    event.send("Request Account ID (Approved)");
  } else {
    event.send("Request Account ID");
  }
  return [(await walletutil.getWallet()).getAddressString()];
}

export const personal_sign = modalutil.interactive(async function(
  message,
  from
) {
  await modalutil.requestSignature.call(this, web3.utils.hexToAscii(message));
  ...
  const serialized = sigUtil.personalSign(
    await walletutil.getPrivateKey(from),
    {
      from: from,
      data: message
    }
...
export const eth_sign = modalutil.interactive(async function(from, message) {
  await modalutil.requestSignature.call(this, web3.utils.hexToAscii(message));
  const serialized = sigUtil.personalSign(
    await walletutil.getPrivateKey(from),
    {
...
export const eth_signTypedData = modalutil.interactive(async function(
  message,
  from,
  extraParams
) {
  await modalutil.requestSignature.call(
    this,
    message
...
export const eth_sendTransaction = modalutil.interactive(async function(
  txParams
) {
  this.setState({ page: "widget", screen: "loading" });

  console.assert(
    (await walletutil.getWallet()).getAddressString() ===
  ...
  if (isERC20) {
    ...
    if (tokenName) {
      ...
      this.setState({
        ...
        insufficientTokens: util
          .BN(tokenMethodParams._value)
          .gt(util.BN(tokenBalance)),
        tokenName: tokenName,
  ...
  if (sufficientLeftovers) {
    event.send("Sufficient Leftover Funds", currentBalance);
    ...
    event.send("Request Funds", priceEstimate);
    this.setState({
      page: "widget",
      screen: "credit"
    });
    ...
    await modalutil.runPaymentFlow.call(this, usdAmount, txParams.from);
  ...
  event.send("Sending Transaction", priceEstimate);
  this.check();
  this.setState({ page: "widget", screen: "finish" });
